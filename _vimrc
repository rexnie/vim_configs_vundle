set nocompatible                            " 禁用 Vi 兼容模式, 放在前面可减少vim报错
let g:cfg_expandtab = 0                     " 1: to expand        0: not to
...
if g:isLinux
  ...
  if g:isGUI
    ...
    set backspace=2                " 设置插入模式时，退格键可删除任意字符，
                                   " 0/1 只可删除刚刚输入的字符 
...
" -----------------------------------------------------------------------------
" {{{
" 光标在buffer i上时,按s 水平分屏打开，按v垂直分屏打开文件
...
if has("cscope")
  ...
  set cscopeverbose
  "快捷键设置
  nnoremap <C-\>s :cs find s <C-R>=expand("<cword>")<CR><CR>"Symbol
  nnoremap <C-\>g :cs find g <C-R>=expand("<cword>")<CR><CR>"Global definition
  nnoremap <C-\>c :cs find c <C-R>=expand("<cword>")<CR><CR>"all Calls to the function
  nnoremap <C-\>t :cs find t <C-R>=expand("<cword>")<CR><CR>"all instances of the Text
  nnoremap <C-\>e :cs find e <C-R>=expand("<cword>")<CR><CR>"Egrep search for the word
  nnoremap <C-\>f :cs find f <C-R>=expand("<cfile>")<CR><CR>"open the Filename
  nnoremap <C-\>i :cs find i ^<C-R>=expand("<cfile>")<CR>$<CR>"files that Include the filename
  nnoremap <C-\>d :cs find d <C-R>=expand("<cword>")<CR><CR>"functions calleD
...
"当noexpandtab设置时,
  "如softtabstop=12,tabstop=8时,那么
    "当输入一个tab时(softtabstop,12列),最后会变成一个tab(tabstop)加4个空格(8+4),
    "当输入两个tab(2个softtabstop,24列),会变成3个tab(tabstop)
  "如softtabstop=4,tabstop=8时,那么
    "当输入一个tab时(softtabstop,4列),最后会变成4个空格(因为不够一个tabstop),
    "当输入两个tab(2个softtabstop,8列),会变成1个tab(tabstop)
  "如softtabstop等于tabstop时，两者没区别

if g:cfg_expandtab
  set expandtab                                         "将Tab键转换为softtabstop个空格，默认noexpandtab
  set tabstop=4                                         "设置Tab键的宽度为4个空格,默认为8
  set softtabstop=4                                     "设置vim把连续数量的空格视为一个制表符,默认softtabstop为8
  set shiftwidth=4                                      "reindent操作(<<和>>)时缩进的空格数,默认为8
endif
...
set cmdheight=1                                     "设置命令行的高度为1，默认为1
...
set undofile
" 保存快捷键
noremap <silent> <F5> :wa!<CR> :mksession! my.vim<CR> :wviminfo! my.viminfo<CR> :echo "Saved"<CR>
" 恢复快捷键
